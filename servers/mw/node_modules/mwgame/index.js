...
function instanceEntitySpawn(oInstance, oEntity) {
	wsh.send(oInstance.getSockets(), 'OC', {
		i: oEntity.getId(), 
		x: oEntity.x, 
		y: oEntity.y, 
		a: oEntity.a, 
...
function instanceEntityRespawn(oInstance, oEntity) {
	...
	wsh.send(oClient.getOtherClientSockets(), 'OC', {
		i: oEntity.getId(), 
		x: oEntity.x, 
		y: oEntity.y, 
		a: oEntity.a, 
...
function init(c) {
	var sServerName = c.name;
	var sServerType = c.type;
	var sServerMotD = c.motd;
	...
	http.defineCommand('mwstatus', function(oResponse, xParam) {
		var oStatus = oMWGame.getStatus(['clients', 'plugins']);
		oStatus.name = c.name;
		oStatus.type = c.type;
		
		var sHTML = '<table>';
		sHTML += '<tr><th>Server</th><td>' + sServerName + '</td></tr>';
		sHTML += '<tr><th>Version</th><td>' + VERSION + '</td></tr>';
		sHTML += '<tr><th>Type</th><td>' + sServerType + '</td></tr>'; 
		sHTML += '<tr><th>Players</th><td>' + oStatus.clients.length + '</td></tr>'; 
		sHTML += '<tr><th>MotD</th><td>' + sServerMotD + '</td></tr>'; 
		sHTML += '</table>';
...
function startMWGame(c) {
	...
	setInterval(oMWGame.process.bind(oMWGame), oMWGame.TIME_FACTOR);
}
...
function service(oSocket) {
	...
	oSocket.on('LI', function(xData) {
		try {
			var sNickname = xData.n;
			// trouver un identifiant
			var id = oMWGame.connectClient(sNickname, oSocket);
			wsh.setData(oSocket, 'id', id);
			serverLog('client identification #', id, '- address:', sAddress, '- name:', sNickname);
			oSocket.emit('ID', { i: id, n: sNickname });
		} catch (e) {
		...
		try {
			...
			oClient.getInstance().getHorde().each(function(e) {
				if (e.id != idEntity) {
					...
					var idClient = e.getData('idClient');
					if (idClient) {
						if (e.getSoul().getAttribute('dead')) {
							return;
						}
						oPacket.n = oMWGame.getClient(idClient).getName();
					} else {
						oPacket.n = e.getData('name');
					}
					oSocket.emit('OC', oPacket);
				}
			});
	...
	oSocket.on('CM', function(xData) {
		try {
			...
			var oClient = oMWGame.getClient(idClient);
			var oInstance = oClient.getInstance();
			serverLog(oInstance.getName(), '> (m)', oClient.getName(), ':', '"' + xData.m + '"');
			wsh.send(oInstance.getSockets(), 'CM', {m: xData.m, n: oClient.getName()});
		} catch (e) {
