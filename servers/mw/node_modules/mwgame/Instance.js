...
O2.createClass('Instance', {
	...
	_oData: null, // données concernant le monde
	...
	loadData: function(sFile) {
		...
		this._oData = this._oMapper.load(sFile);
		// hum hum .map.map
		// heureusement que _oData n'est pas un array
		// le premier map est la propriété de oData (c'est un Array).
		// le second map est la fonction d'itération de cet Array
		this._aMapProp = this._oData.map.map(function(row, y) {
		...
		var getLongestView = function(x, y) {
			...
			oViews.forEach(function(v) {
				var bx = x;
				var by = y;
				while (oThis.getMapBlockPhys(bx, by) == BLOCK.VOID) {
					bx += v.dx;
					by += v.dy;
					++v.dist;
				}
	...
	process: function(bGetState) {
		...
		if (aDestroy) { // y'a une liste des morts ?
			...
			aDestroy.forEach(this.destroyEntity.bind(this));
		}
	...
	spawnEntity: function(sBlueprint, x, y, a) {
		...
		if (sBlueprint) {
			...
			oEntity.onEnterSector = this.entityEnteringSector.bind(this);
			oEntity.move(x, y);
			oEntity.setViewAngle(a);
			oEntity.setCreationTime(this.getTime());
			oEntity.onLeaveSector = this.entityLeavingSector.bind(this);
	...
	getMapBlock: function(x, y) {
		return this._oData.map[y][x];
	},
	
	getMapBlockPhys: function(x, y) {
		return (this._oData.map[y][x] & 0xFF00) >> 8;
	},
	...
	setMapBlock: function(x, y, b, sTag) {
		this._oData.map[y][x] = b;
		if (sTag && this.onMapChange) {
			this.onMapChange(this, x, y, b, sTag);
		}
	},
	
	setMapBlockPhys: function(x, y, b) {
		this._oData.map[y][x] = (this._oData.map[y][x] & 0xFFFF00FF) | (b << 8);
	},
	...
	getData: function() {
		...
		var oMapData = {m: this._oData, d: aDoorData};
		return oMapData;
	},
