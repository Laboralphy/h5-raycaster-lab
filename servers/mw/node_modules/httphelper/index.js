	...
	var oRequest = http.get(oOptions, function(oResponse) {
		...
		oResponse.on('end', function() {
			if (oResponse.statusCode >= 500) {
				pCallback('error ' + oResponse.statusCode, '');
			} else {
...
function httpCommand(oRequest, oResponse, oPostData) {
	try {
		...
		if (sCommand in oCommands) {
			oCommands[sCommand](oResponse, oParse.query, oPostData);
			return true;
		} else {
			return false;
		}
	} catch(e) {
		httpError(500, e.toString(), oResponse);
	}
}
...
function httpGetPostData(oRequest, oResponse, pCallBack) {
	...
	oRequest.on('end', function(sData) {
