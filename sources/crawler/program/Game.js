O2.extendClass('Game', O876_Raycaster.Engine, {
	...
	onInitialize : function() {
		...
		this.oPathFinder.isCellWalkable = function(x, y) {		
			try {
				return (this.getCell(x, y) & 0xFF00) === 0; // code12: (this.getCell(x, y) & 0xF000) === 0
			} catch (e) {
				return false;
			}
		},
...
//		var nFloor = this.oDungeon.getPlayerLocationFloor();
		...
		for (yBlock = 0; yBlock < aLaby.length; yBlock++) {
			...
			for (xBlock = 0; xBlock < aLaby[yBlock].length; xBlock++) {
				...
				if (oData.walls.codes[nCode & 0xFF] === undefined) {  // code12: oData.walls.codes[nCode & 0xFFF]
					throw new Error('code: 0x' + (nCode & 0xFF).toString(16) + ' is not defined');// code12: 'code: 0x' + (nCode & 0xFFF).toString(16) + ' is not defined'
				}
				aRow.push(nCode);
			}
			oData.map.push(aRow);
